    ...
    for path in raw_file_paths:
        ...
        for source in sources:
            if source.lower().endswith(".csv"):
                con.execute(f"CREATE OR REPLACE TEMP VIEW read_file_raw AS SELECT * FROM read_csv('{source}', ignore_errors=true, delim=',', quote='\"', escape='\"', sample_size=-1, max_line_size=10000000, strict_mode=false, null_padding=true)")
