...
def _run(con, raw_file_path: str, cell_reference_path: str, temp_csv: str | None) -> str:
    ...
    try:
        con.execute(f"CREATE OR REPLACE TEMP VIEW raw AS SELECT * FROM read_csv('{csv_path}', ignore_errors=true, delim=',', quote='\"', escape='\"', sample_size=-1, max_line_size=10000000, strict_mode=false, null_padding=true)")
